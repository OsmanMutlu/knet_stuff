...
=#
    ...
    for i in 1:epochs
        ...
        if acc1 > best_acc
            JLD.save("experiment_with_vocab20k_best_model_only_word_embeds.jld", "model", model, "optims", opts)
            best_acc = acc1
    ...
    JLD.save("experiment_with_vocab20k_only_word_embeds.jld", "model", model, "optims", opts)
    #println("Final!!! Loss val $lss trn acc $trnacc tst acc $acc1 ...")

    test_df = load("testData_20180901_only_word_embeds.jld")["data"]

    if acc1 < best_acc
        asd = load("experiment_with_vocab20k_best_model_only_word_embeds.jld")
        model = asd["model"]
        opts = asd["optims"]
...
#JLD.save("vocab20k_latest.jld", "vocab", vocab)
vocab = load("vocab20k_latest.jld")["vocab"]
